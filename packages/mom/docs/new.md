...
```typescript
import { type AgentSessionEvent } from "open-pi-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "open-pi-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `open-pi-ai` and `open-pi-mom` are aliased to the running mom bundle.
