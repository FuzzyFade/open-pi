...
Stateful agent with tool execution and event streaming. Built on `open-pi-ai`.

## Installation

```bash
npm install open-pi-agent
```
...
```typescript
import { Agent, streamProxy } from "open-pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "open-pi-agent";
