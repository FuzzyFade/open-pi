...
- Fixed `open-pi-ai/oauth` package exports to point directly at built `dist` files, avoiding broken TypeScript resolution through unpublished wrapper targets ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level package entry. Import OAuth login/refresh functions from `open-pi-ai/oauth` instead of `open-pi-ai` ([#1814](https://github.com/badlogic/pi-mono/issues/1814))
...
- Re-exported TypeBox `Type`, `Static`, and `TSchema` from `open-pi-ai` to match documentation and avoid duplicate TypeBox type identity issues in pnpm setups ([#1338](https://github.com/badlogic/pi-mono/issues/1338))
